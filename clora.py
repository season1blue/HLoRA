...
class CLoraWrapper(nn.Module):
    ...
    def prepare_regulariation_matrix(self):
        ...
        target_modules = self.peft_model.peft_config[self.adapter_name].target_modules
        ...
        for n, v_ in self.peft_model.model.named_modules():
            ...
            if not hasattr(v_, 'weight'):
                continue

            # 检查模块名称是否在 target_modules 中
            if not any(target in n for target in target_modules):
                continue
            v = v_.weight
            # print(f"n:{n}")

            # print(f"v_.type{v.dtype}")
            
            # output, k
            self.u[n.replace('.', '-')] = v.new_zeros(v.size(0), self.k,dtype=torch.float32) # parameter dict do not allow . in key
            # input, k dtype=torch.float32
            self.v[n.replace('.', '-')] = v.new_zeros(v.size(1), self.k,dtype=torch.float32)

...
class EWCLoRAModel(torch.nn.Module):
    def __init__(self, model_name_or_path, fisher_matrix_path, accelerator, peft_config, ewc_lambda=1):
        super().__init__()
...
def main(
    ...
    model = EWCLoRAModel(model_name_or_path, fisher_matrix_path, accelerator, peft_config, ewc_lambda=ewc_lambda)
    ...
    for epoch in range(B_num_epochs):
        ...
        for batch in B_train_dataloader:
            with accelerator.accumulate(model):
                ...
                # 更新 tqdm 描述信息
                progress_bar.set_description(f"TaskB: Epoch {epoch} - Loss: {loss_num:.4f}, CE Loss: {ce_loss:.4f}, EWC Loss: {ewc_loss:.4f}")
    ...
    for epoch in range(C_num_epochs):
        ...
        for batch in C_train_dataloader:
            with accelerator.accumulate(model):
                ...
                ewc_loss = outputs.ewc_loss.detach().cpu().item()
                # 更新 tqdm 描述信息
                progress_bar.set_description(f"TaskC: Epoch {epoch} - Loss: {loss_num:.4f}, CE Loss: {ce_loss:.4f}, EWC Loss: {ewc_loss:.4f}")
                progress_bar.update(1)
    ...
    #             ewc_loss = outputs.ewc_loss.detach().cpu().item()
    #             # 更新 tqdm 描述信息
    #             progress_bar.set_description(f"TaskD: Epoch {epoch} - Loss: {loss_num:.4f}, CE Loss: {ce_loss:.4f},EWC Loss: {ewc_loss:.4f}")
    #             progress_bar.update(1)
    ...
    #     train_epoch_loss = total_loss / len(D_train_dataloader)
    #     accelerator.print(f"{epoch=}: {train_epoch_loss=}")

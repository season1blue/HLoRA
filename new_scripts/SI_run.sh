...
REG_CONF=1e-3                                             #正则化系数
IS_LAYERWISE="false"                                           #"false" is no layerwise
########################################################################################
...
# 根据模型选择不同的脚本
if [[ "$IS_LAYERWISE" == "false" ]]; then
    SCRIPT="SI_run_no_layer.py"
    LOG_FILE="log/slora-$MODEL_NAME-$DATASET-r$LORA_RANK-C$REG_CONF-nolayerwise.log"
    OUTPUT_DIR="output/slora-$MODEL_NAME-$DATASET-r$LORA_RANK-C$REG_CONF-nolayerwise/checkpoint"               
else
    SCRIPT="SI_run.py"
    LOG_FILE="log/slora-$MODEL_NAME-$DATASET-r$LORA_RANK-C$REG_CONF-layerwise.log"
    OUTPUT_DIR="output/slora-$MODEL_NAME-$DATASET-r$LORA_RANK-C$REG_CONF-layerwise/checkpoint"     
fi

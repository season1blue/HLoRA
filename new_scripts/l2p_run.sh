...
MODEL="l2p"                                              # 可以修改为 "jslora" 或 "ewclora" 或 "rslora" 来运行对应的模型
...
elif [[ "$MODEL" == "clora" ]]; then
    SCRIPT="clora_run.py"
    LOG_FILE="log/clora-$MODEL_NAME-$DATASET-r$LORA_RANK.log"
    OUTPUT_DIR="output/clora-$MODEL_NAME-$DATASET-r$LORA_RANK"
elif [[ "$MODEL" == "l2p" ]]; then
    SCRIPT="L2P_run.py"
    LOG_FILE="log/l2p-$MODEL_NAME-$DATASET-r$LORA_RANK.log"
    OUTPUT_DIR="output/l2p-$MODEL_NAME-$DATASET-r$LORA_RANK"            
else

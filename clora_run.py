...
class EWCLoRAModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        # loss = outputs[0]  # 提取 CLoraWrapper 返回的 loss
        # logits = outputs[1]  # 假设 logits 是元组的第二个元素（根据实际情况调整索引）
        print(f"outputs0.shape:{outputs[0].shape}")
        # print(f"outputs1:{outputs[1]}")

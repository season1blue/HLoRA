...
class SILoRAModel(torch.nn.Module):
    ...
    def calculate_importance(self, task="None"):  #涉及None阶段初始化Importance，以及A阶段计算A任务的Importance
            ...
            if task=="None": # The case of the first task
            ...
            elif task=="B":    # The case of after the second task
                importance = self.regularization_terms["A"]['importance']
                prev_params = self.regularization_terms["A"]['task_param']
                ...
                self.regularization_terms["B"] = {'importance': importance, 'task_param': prev_params}  # 留个下个任务用

            elif task=="C":    # The case of after the second task
                importance = self.regularization_terms["B"]['importance']
                prev_params = self.regularization_terms["B"]['task_param']
                ...
                self.regularization_terms["C"] = {'importance': importance, 'task_param': prev_params}  # 留个下个任务用
            
            elif task=="D":    # The case of after the second task
                importance = self.regularization_terms["C"]['importance']
                prev_params = self.regularization_terms["C"]['task_param']
                ...
                self.regularization_terms["D"] = {'importance': importance, 'task_param': prev_params}  # 留个下个任务用
    ...
    def save_importance(self, importance_name):
            ...
            elif "B" in self.regularization_terms.keys():
                importance_dict = self.regularization_terms["B"]['importance']
            elif "C" in self.regularization_terms.keys():
                importance_dict = self.regularization_terms["C"]['importance']
            elif "D" in self.regularization_terms.keys():
                importance_dict = self.regularization_terms["D"]['importance']
            else:
...
def main(
    ...
    A_lr_scheduler = get_linear_schedule_with_warmup(
        optimizer=optimizer,
        num_warmup_steps=0,
        num_training_steps=(len(A_train_dataloader) * A_num_epochs),
    )
   ...
   # Step 2: Calculate the importance of  Task A
        ...
        model.save_importance(
            importance_name=f"importance_{A_template_name}_gptj_{reg_conf}"
            ...
            for batch in B_train_dataloader:
                with accelerator.accumulate(model):
                    ...
                    for index, (n, p) in enumerate(dynamic_params.items()):
                        ...
                        task_reg_loss += curr_norm * (importance[n] * (p - task_param[n]) ** 2).sum()
   ...
   # Step 4: Calculate the importance of  Task B
            model.calculate_importance(task="B")
            model.save_importance(
                importance_name=f"importance_{B_template_name}_gptj_{reg_conf}"
                ) 
            end_time = time.time()
    ...
    for epoch in range(C_num_epochs):
            ...
            for batch in C_train_dataloader:
                with accelerator.accumulate(model):
                    ...
                    importance = model.regularization_terms["B"]['importance']
                    task_param = model.regularization_terms["B"]['task_param']
                    ...
                    # 更新 tqdm 描述信息
                    progress_bar.set_description(f"TaskC: Epoch {epoch} - Loss: {loss:.4f},Ce_Loss: {loss_num:.4f} Reg_Loss: {reg_loss:.4f}")
                    progress_bar.update(1)
   ...
   # Step 6: Calculate the importance of  Task C
            model.calculate_importance(task="C")
            model.save_importance(
                importance_name=f"importance_{C_template_name}_gptj_{reg_conf}"
                ) 
            end_time = time.time()
            elapsed_time = end_time - start_time
            accelerator.print(f"Started: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}, Ended: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}, Duration: {elapsed_time:.2f} seconds")
...
#    # Step 7 : Train Task D
#     with accelerator.main_process_first():
#         D_train_dataset = D_train_dataset.map(
#             partial(preprocess_function, template_name=D_template_name),
#             batched=True,
#             num_proc=1,
#             remove_columns=D_train_dataset.column_names,
#             load_from_cache_file=True,
#             desc="Running tokenizer on dataset",
#         )
#     accelerator.wait_for_everyone()

#     D_data_total_size = len(D_train_dataset)
#     if D_data_total_size > 20000:
#         indices = np.random.choice(D_data_total_size, 20000, replace=False)
#         D_train_dataset =Subset(D_train_dataset,indices)


#     D_train_dataloader = DataLoader(
#         D_train_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=per_device_train_batch_size, pin_memory=True
#     )

#     D_lr_scheduler = get_linear_schedule_with_warmup(
#         optimizer=optimizer,
#         num_warmup_steps=0,
#         num_training_steps=(len(D_train_dataloader) * D_num_epochs),
#     )

#     model, D_train_dataloader, optimizer, D_lr_scheduler = accelerator.prepare(
#         model, D_train_dataloader, optimizer, D_lr_scheduler
#     )
...
#             start_time = time.time()
#             total_steps = len(D_train_dataloader)
#             progress_bar = tqdm(range(total_steps), disable=not accelerator.is_local_main_process, ncols=200)
...
#                     importance = model.regularization_terms["C"]['importance']
#                     task_param = model.regularization_terms["C"]['task_param']
...
#                     # 更新 tqdm 描述信息
#                     progress_bar.set_description(f"TaskD: Epoch {epoch} - Loss: {loss:.4f},Ce_Loss: {loss_num:.4f} Reg_Loss: {reg_loss:.4f}")
#                     progress_bar.update(1)
...
#             # Saving model
#             epoch_output_dir = os.path.join(output_dir, f"{D_template_name}_epoch_{epoch + 1}")
#             os.makedirs(epoch_output_dir, exist_ok=True)
#             accelerator.print(f"Saving model to {epoch_output_dir}...")
#             accelerator.unwrap_model(model).save_pretrained(epoch_output_dir)
